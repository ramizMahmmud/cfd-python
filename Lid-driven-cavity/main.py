...
def main():
    DX = DOMAIN_SIZE / (NODES - 1)
    x = np.linspace(0.0, DOMAIN_SIZE, NODES)
    y = np.linspace(0.0, DOMAIN_SIZE, NODES)
    ...
    if DT > STABILITY_SAFETY_FACTOR * maximum_possible_time_step_length:
    ...
    plt.xlim((0, 1))
    plt.ylim((0, 1))
    plt.savefig("lid_driven_cavity.png", dpi=300)
    plt.show()
